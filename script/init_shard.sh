...
config = { _id: "s0", members:[
          ...
          { _id : 1, host : "localhost:37018" ,priority:0},
          { _id : 2, host : "localhost:37019" ,priority:1}]};
rs.initiate(config)
EOF
...
config = { _id: "s1", members:[
          { _id : 0, host : "localhost:47017",priority:2},
          { _id : 1, host : "localhost:47018" ,priority:0},
          { _id : 2, host : "localhost:47019" ,priority:1}]};
rs.initiate(config)
EOF
...
config = { _id: "s2", members:[
          { _id : 0, host : "localhost:57017",priority:2 },
          { _id : 1, host : "localhost:57018" ,priority:0},
          { _id : 2, host : "localhost:57019" ,priority:1}]};
rs.initiate(config)
EOF
...
mongos --logpath "mongos-1.log" --chunkSize 1 --configdb localhost:57040,localhost:57041,localhost:57042 --fork
echo "Waiting 60 seconds for sharding fully come online"
sleep 60
...
db.adminCommand({enableSharding: "college"})
db.adminCommand({shardCollection: "college.students", key: {student_id:1}});
EOF
